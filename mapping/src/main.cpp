...
{   
    ...
    std::string settingsFilename = "mapperSettings.xml";
    if (argc == 2)
    {
    ...
    std::string altitudesString = readNodeAndDefaultIfNonexistent("GridAltitudes", settings, std::string("0"));
    ...
    std::istringstream altitudesStream(altitudesString);
    std::ostringstream altitudesStreamOut;
    vector<double> altitudes;
    double altitude;
    while(altitudesStream >> altitude)
    {
        altitudes.push_back(altitude);
        altitudesStreamOut << altitude << " ";
    }
    std::cout << "time: ";
    ...
    bool outputGNUPlotHeatmapData = (bool)readNodeAndDefaultIfNonexistent("OutputGNUPlotHeatmapData",settings,int(0));
    bool outputHDOPFourier = (bool)readNodeAndDefaultIfNonexistent("OutputHDOPFourier",settings,int(0));
    std::string outputHDOPFilename = readNodeAndDefaultIfNonexistent("OutputHDOPFilename",settings,std::string("hdop.png"));
    std::string outputHDOPFourierFilename = readNodeAndDefaultIfNonexistent("OutputHDOPFourierFilename",settings,std::string("hdop-fourier.png"));
    if (outputGNUPlotHeatmapData)
    {
        cout << "\"x\""
             << "\t" << "\"y\"";
        ...
        for (int x=0; x<mapProperties.m_num_cols; x++)
        {
            for (int y=0; y<mapProperties.m_num_rows; y++)
            {
                cout        << x 
                    << "\t" << y;
                for (int z=0; z<altitudes.size(); z++)
                {
                    double hdop = dopMap.m_items[z][x][y].getHorizontal();
                    cout << "\t" << hdop;
                }
                cout << endl;
            }
    ...
    //*
        ...
        // imwrite(outputHDOPFourierFilename, magI);
        ...
        for (auto item:circular)
        {
            ...
            if (values.size()) sum /= values.size();
            cout << r << "\t" << sum << endl;
        }
